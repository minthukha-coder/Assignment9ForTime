...
// from-WinHtut-to-lonelone-100
    ...
    if(who == 't'){
        ...
        int time_counter = charCounting(buffer);
        for (int i = 0;i < time_counter;i++) {
            db[transfer].trc[space_array[transfer]- 15].note[index_point] = buffer[i];
            index_point++;
        }
        ...
        int time_counter = charCounting(buffer);
        for (int i = 0;i < time_counter;i++) {
            db[receiver].trc[space_array[receiver]- 15].note[index_point] = buffer[i];
            index_point++;
        }
